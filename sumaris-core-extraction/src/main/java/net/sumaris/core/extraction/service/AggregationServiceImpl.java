...
public class AggregationServiceImpl implements AggregationService {
    ...
    public AggregationContextVO execute(AggregationTypeVO type, ExtractionFilterVO filter) {
        ...
        switch (category) {
            case PRODUCT:
                // Get the product VO
                source = extractionProductRepository.getByLabel(checkedType.getFormat(), ProductFetchOptions.MINIMAL);
    ...
    public AggregationResultVO read(AggregationTypeVO type, @Nullable ExtractionFilterVO filter, @Nullable AggregationStrataVO strata, int offset, int size, String sort, SortDirection direction) {
        ...
        ExtractionProductVO product = extractionProductRepository.getByLabel(type.getLabel(), ProductFetchOptions.MINIMAL_WITH_TABLES);
    ...
    public AggregationTypeVO save(AggregationTypeVO type, @Nullable ExtractionFilterVO filter) {
        ...
        if (type.getId() != null) {
            target = extractionProductRepository.findById(type.getId(), ProductFetchOptions.FOR_UPDATE).orElse(null);
        }
    ...
    public List<ExtractionProductColumnVO> getColumnsBySheetName(AggregationTypeVO type, String sheetName) {
        ...
        if (productId == null) {
            ExtractionProductVO product = extractionProductRepository.getByLabel(type.getLabel(), ProductFetchOptions.MINIMAL);
