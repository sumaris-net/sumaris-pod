...
public class VesselSnapshotServiceImpl implements VesselSnapshotService {
	...
	public List<VesselSnapshotVO> findAll(@NonNull VesselFilterVO filter,
		...
		try {
			if (isElasticsearchEnableAndReady()) {
				List<VesselSnapshotVO> result = elasticsearchRepository.findAll(filter, page, fetchOptions);
				return result;
			}
	...
	protected void indexVesselSnapshots(
		...
		try {
			...
			final Set<Integer> existingIds = filter.getMinUpdateDate() != null
				? Beans.getSet(elasticsearchRepository.findAllIds())
				: null;
				...
				do {
					try {
						...
						items.forEach(vessel -> {
							...
							if (existingIds == null || !existingIds.remove(vessel.getId())) {
								inserts.increment();
							}
							else {
								updates.increment();
