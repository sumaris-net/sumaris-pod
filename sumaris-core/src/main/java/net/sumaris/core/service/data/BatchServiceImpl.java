...
public class BatchServiceImpl implements BatchService {
    ...
    public List<BatchVO> saveAllByOperationId(int operationId, List<BatchVO> sources) {
        ...
        if (this.enableImages) {
            result.forEach(this::saveImageAttachments);
        }
    ...
    public BatchVO save(BatchVO batch) {
        ...
        BatchVO result = batchRepository.save(batch);
    ...
    private void saveImageAttachments(BatchVO source) {
        if (source.hasNotFlag(ValueObjectFlags.SAME_HASH)) {
            List<ImageAttachmentVO> images = Beans.getList(source.getImages());
            images.forEach(image -> fillDefaultProperties(source, image));
            images = imageAttachmentService.saveAllByParent(images, source, ObjectTypeEnum.BATCH);
    ...
    protected void fillDefaultProperties(BatchVO parent, ImageAttachmentVO image) {
        ...
        if (image.getDateTime() == null) {
            image.setDateTime(parent.getUpdateDate());
        }

        // Link to sample
        image.setObjectId(parent.getId());
        image.setObjectTypeId(ObjectTypeEnum.BATCH.getId());
    }
}
