...
@EntityEnum(entity = Parameter.class, resolveAttributes = {ParameterGroup.Fields.LABEL, IEntity.Fields.ID})
public enum ParameterEnum implements Serializable, IEntityEnum {
    ...
    public static ParameterEnum valueOf(final int id) {
        return Arrays.stream(values())
            .filter(methodEnum -> methodEnum.id == id)
            .findFirst()
            .orElseThrow(() -> new IllegalArgumentException("Unknown ParameterEnum: " + id));
    }

    private Integer id;
    private String label;

    ParameterEnum(Integer id, String label) {
        this.id = id;
        this.label = label;
    }
