...
/***/ (function(module, exports, __webpack_require__) {
	...
	module.exports = function ( graph ){
	  ...
	  function appendIriLabel( element, name, iri ){
		...
		// TODO BLA
		if ( iri && window.location.toString().startsWith("http") && window.location.hash.startsWith("#iri=")) {
			var slashIndex = iri.lastIndexOf('#');
			var baseIri = slashIndex !== -1 ? iri.substr(0, slashIndex) : iri; // remove last slash
			if (baseIri.startsWith("http")) {
				console.info("Detecting click on an element from iri: ", baseIri);
				var existsInView = window.location.hash.indexOf(baseIri) !== -1;
				element.append("span").text(" ");
				var actions = element.append("small");
				if (!existsInView) {

					var addHref = (window.location.hash + "+" + baseIri).replace("\+\+", "+");
					actions.append("a")
						.attr("href", addHref)
						.attr("title", "Add to current view")
						.attr("style", "color: yellow")
						.text("add");
				}
				else {
					var removeHref = window.location.hash.replace(baseIri, "")
						.replace("\+\+", "+")
						.replace("=\+", "=");
					actions.append("a")
						.attr("href", removeHref)
						.attr("style", "color: yellow")
						.attr("title", "Remove from current view")
						.text("remove");
				}
				...
				actions.append("a")
					.attr("href", "#iri=" + baseIri)
					.attr("title", "View (in a new tab)")
					.attr("target", "_blank")
					.text("view");
