...
with
    ...
    effort_t1_rea(strat_id, T1_REA) AS (select strat.id, count(sa.id)
                                        ...
                                        where s.strategy_fk = strat.id and sa.program_fk = 'SIH-PARAM-BIO' and sa.fishing_operation_fk = o.id and o.fishing_trip_fk = l.fishing_trip_fk
                                          and lm.landing_fk = l.id and lm.pmfm_fk = 1389 and lm.alphanumerical_value = strat.code    and sa.sample_date between ap.start_date and ap.end_date
                                          and to_char(ap.start_date,'DD/MM') = '01/01'and s.id = ap.applied_strategy_fk
                                        GROUP BY strat.id
    ),
    effort_t1_rea_cam(strat_id, T1_REA) AS (select strat.id, count(SA.id) FROM sample SA, SAMPLE_MEASUREMENT sm, strat
                                            WHERE sm.PMFM_FK = 2792 AND SA.ID = sm.SAMPLE_FK AND sm.ALPHANUMERICAL_VALUE LIKE ('%' || strat.code || '%')
                                              AND To_CHAR(SA.SAMPLE_DATE , 'Q') = '1'
                                            GROUP BY strat.id
    ),
    ...
    effort_t2_rea(strat_id, T2_REA) AS (select strat.id, count(sa.id)
                                        from applied_period ap, applied_strategy s, sample sa, landing_measurement lm, landing l, operation o, strat
                                        where
                                                s.strategy_fk = strat.id
                                          and sa.program_fk = 'SIH-PARAM-BIO' and sa.fishing_operation_fk = o.id and o.fishing_trip_fk = l.fishing_trip_fk
                                          and lm.landing_fk = l.id and lm.pmfm_fk = 1389 and lm.alphanumerical_value = strat.code    and sa.sample_date between ap.start_date and ap.end_date
                                          and to_char(ap.start_date,'DD/MM') = '01/04'and s.id = ap.applied_strategy_fk
                                        GROUP BY strat.id
    ),
    effort_t2_rea_cam(strat_id, T2_REA) AS (select strat.id, count(SA.id) FROM sample SA, SAMPLE_MEASUREMENT sm, strat
                                            WHERE sm.PMFM_FK = 2792 AND SA.ID = sm.SAMPLE_FK AND sm.ALPHANUMERICAL_VALUE LIKE ('%' || strat.code || '%')
                                              AND To_CHAR(SA.SAMPLE_DATE , 'Q') = '2'
                                            GROUP BY strat.id
    ),
    ...
    effort_t3_rea(strat_id, T3_REA) AS (select strat.id, count(sa.id)
                                        ...
                                        where s.strategy_fk = strat.id and sa.program_fk = 'SIH-PARAM-BIO' and sa.fishing_operation_fk = o.id and o.fishing_trip_fk = l.fishing_trip_fk
                                          and lm.landing_fk = l.id and lm.pmfm_fk = 1389 and lm.alphanumerical_value = strat.code    and sa.sample_date between ap.start_date and ap.end_date
                                          and to_char(ap.start_date,'DD/MM') = '01/07'and s.id = ap.applied_strategy_fk
                                        GROUP BY strat.id
    ),
    effort_t3_rea_cam(strat_id, T3_REA) AS (select strat.id, count(SA.id) FROM sample SA, SAMPLE_MEASUREMENT sm, strat
                                            WHERE sm.PMFM_FK = 2792 AND SA.ID = sm.SAMPLE_FK AND sm.ALPHANUMERICAL_VALUE LIKE ('%' || strat.code || '%')
                                              AND To_CHAR(SA.SAMPLE_DATE , 'Q') = '3'
                                            GROUP BY strat.id
    ),
    ...
    effort_t4_rea(strat_id, T4_REA) AS (select strat.id, count(sa.id)
                                        ...
                                        where s.strategy_fk = strat.id and sa.program_fk = 'SIH-PARAM-BIO' and sa.fishing_operation_fk = o.id and o.fishing_trip_fk = l.fishing_trip_fk
                                          and lm.landing_fk = l.id and lm.pmfm_fk = 1389 and lm.alphanumerical_value = strat.code    and sa.sample_date between ap.start_date and ap.end_date
                                          and to_char(ap.start_date,'DD/MM') = '01/10'and s.id = ap.applied_strategy_fk
                                        GROUP BY strat.id
    ),
    effort_t4_rea_cam(strat_id, T4_REA) AS (select strat.id, count(SA.id) FROM sample SA, SAMPLE_MEASUREMENT sm, strat
                                            WHERE sm.PMFM_FK = 2792 AND SA.ID = sm.SAMPLE_FK AND sm.ALPHANUMERICAL_VALUE LIKE ('%' || strat.code || '%')
                                              AND To_CHAR(SA.SAMPLE_DATE , 'Q') = '4'
                                            GROUP BY strat.id
    ),
