...
with
    ...
    effort_t1_rea(strat_id, T1_REA) AS (select strat.id, count(sa.id)
                                        ...
                                        where s.strategy_fk = strat.id and sa.program_fk = 'SIH-PARAM-BIO' and s.id = ap.applied_strategy_fk
                                          ...
                                          and lm.landing_fk = l.id and lm.pmfm_fk = 2867 and lm.alphanumerical_value = strat.code
    ...
    effort_t1_rea_cam(strat_id, T1_REA) AS (select strat.id, count(SA.id)
                                            FROM applied_period ap, applied_strategy s, sample sa, SAMPLE_MEASUREMENT sm, strat
                                            WHERE s.strategy_fk = strat.id and sa.program_fk = 'SIH-PARAM-BIO' and s.id = ap.applied_strategy_fk
                                              and extract(MONTH FROM ap.start_date) = 1
                                              AND sm.PMFM_FK = 1435 AND sa.ID = sm.SAMPLE_FK AND sm.ALPHANUMERICAL_VALUE LIKE (strat.code || '%')
                                              AND sa.sample_date between ap.start_date and (ap.end_date + INTERVAL '1' DAY)
                                            GROUP BY strat.id
    ),
...
select TO_CHAR(END_DATE, 'DD/MM/YYYY HH24:MI:SS') from SIH2_ADAGIO_DBA.APPLIED_PERIOD