...
export class PmfmStrategyFilter extends EntityFilter<PmfmStrategyFilter, PmfmStrategy> {
  ...
  buildFilter(): FilterFn<PmfmStrategy>[] {
    ...
    if (this.acquisitionLevel) {
      ...
      filterFns.push(
        (t) => (EntityUtils.isNotEmpty(t.acquisitionLevel, 'label') ? t.acquisitionLevel['label'] : t.acquisitionLevel) === acquisitionLevel
      );
    }
...
export class PmfmStrategiesTable extends AppInMemoryTable<PmfmStrategy, PmfmStrategyFilter> implements OnInit {
  ...
  ngOnInit() {
    ...
    this.registerFormField(
      'defaultQualitativeValue',
      {
        type: 'entity',
        autocomplete: {
          attributes: qvAttributes,
          items: this.onStartEditingRow.pipe(
            switchMap((row) => {
            ...
            debounceTime(200),
            map((pmfm) =>
              isNotEmptyArray(pmfm && pmfm.qualitativeValues) ? pmfm.qualitativeValues : (pmfm.parameter && pmfm.parameter.qualitativeValues) || []
            )
          ),
          showAllOnFocus: false,
          class: 'mat-autocomplete-panel-large-size',
  ...
  protected async loadReferential() {
    ...
    try {
      await Promise.all([this.loadAcquisitionLevels()]);
