  ...
  loadEffort: gql`
    query StrategyEffort(
      ...
      data: extraction(
        ...
        filter: { sheetName: $viewSheetName, criteria: [{ sheetName: $filterSheetName, name: $columnName, operator: $operator, values: $values }] }
      )
    }
  `,
};
...
export class SamplingStrategyService extends BaseReferentialService<SamplingStrategy, StrategyFilter> {
  ...
  watchAll(
    ...
    return (
      super
        ...
        .pipe(mergeMap((res) => this.fillEntities(res, opts)))
    );
  }
  ...
  protected async fillParameterGroups(entities: SamplingStrategy[]): Promise<void> {
    ...
    entities.forEach((s) => {
      const pmfms = s.pmfms;
      s.parameterGroups = ((pmfms && parameterListKeys) || []).reduce(
        (res, key) =>
          pmfms.findIndex((p) => pmfmIdsMap[key].includes(p.pmfmId) || (p.parameter && p.parameter.label && p.parameter.label.includes(key))) !== -1
            ? res.concat(key)
            : res,
        []
      );
  ...
  async fillEfforts(
    ...
    (data || []).map(StrategyEffort.fromObject).forEach((effort) => {
      ...
      if (strategy) {
        ...
        console.warn(
          `[denormalized-strategy-service] An effort has unknown strategy '${effort.strategyLabel}'. Skipping. Please check GraphQL query 'extraction' of type 'strat'.`
        );
      }
    });
  }
