...
const StrategySubscriptions = {
  listenChangesByProgram: gql`
    subscription UpdateProgramStrategies($programId: Int!, $interval: Int) {
        ...
        subscription: this.graphql
          ...
          .pipe(
            map(({ data }) => {
              ...
              if (isNotEmptyArray(entities) && this._debug)
                console.debug(`[strategy-ref-service] [WS] Received changes on strategies, from program {${programId}}`, entities);
              return entities;
            })
          )
          .subscribe(subject),
