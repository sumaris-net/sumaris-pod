...
export const ProgramRefQueries = {
  ...
  loadAllWithTotalAndStrategy: gql`
    query Programs($filter: ProgramFilterVOInput!, $offset: Int, $size: Int, $sortBy: String, $sortDirection: String) {
      data: programs(filter: $filter, offset: $offset, size: $size, sortBy: $sortBy, sortDirection: $sortDirection) {
        ...ProgramRefFragment
        strategies {
...
const ProgramRefSubscriptions: BaseEntityGraphqlSubscriptions = {
  listenChanges: gql`
    subscription UpdateProgram($id: Int, $label: String, $interval: Int) {
  ...
  implements IEntitiesService<Program, ProgramFilter>, IEntityService<Program>
{
              ...
              (p) =>
                ...
                (!opts ||
                  ((!opts.acquisitionLevel || p.acquisitionLevel === opts.acquisitionLevel) &&
                    ...
                    (!opts.taxonGroupId ||
                      !p.taxonGroupIds ||
                      !p.taxonGroupIds.length ||
                      p.taxonGroupIds.findIndex((g) => g === opts.taxonGroupId) !== -1) &&
                    // Filter on reference taxon
                    (!opts.referenceTaxonId ||
                      !p.referenceTaxonIds ||
                      !p.referenceTaxonIds.length ||
                      p.referenceTaxonIds.findIndex((g) => g === opts.referenceTaxonId) !== -1) &&
                    // Add to list of IDs
                    pmfmIds.push(p.id)))
            )
  ...
  watchTaxonGroups(
    ...
    // Watch program
    return this.watchByLabel(programLabel, { toEntity: false }).pipe(
      map((program) => {
        // Find strategy
        const strategy = ((program && program.strategies) || []).find((s) => !opts || !opts.strategyLabel || s.label === opts.strategyLabel);
  ...
  async executeImport(
    ...
    try {
      ...
      if (opts && isNotEmptyArray(opts.acquisitionLevels)) {
        const acquisitionLevels: string[] =
          (opts && opts.acquisitionLevels) || Object.keys(AcquisitionLevelCodes).map((key) => AcquisitionLevelCodes[key]);
