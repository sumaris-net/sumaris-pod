...
import { fromDateISOString, isNil, isNilOrBlank, isNotNil, isNotNilOrNaN, joinPropertiesPath, ReferentialRef, ReferentialUtils, toDateISOString } from '@sumaris-net/ngx-components';
    ...
    switch (pmfm.type) {
      case 'qualitative_value':
        if (isNotNil(value)) {
          ...
          return (
            (pmfm.qualitativeValues || (PmfmUtils.isFullPmfm(pmfm) && pmfm.parameter && pmfm.parameter.qualitativeValues) || []).find(
              (qv) => qv.id === qvId
            ) || null
          );
        }
