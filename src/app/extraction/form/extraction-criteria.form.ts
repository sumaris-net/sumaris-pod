  ...
  implements OnInit
{
  ...
  get criteriaCount(): number {
    return Object.values(this.form.controls)
      ...
      .map(
        (sheetForm) =>
          sheetForm.controls.map((criterionForm) => (criterionForm as FormGroup).value).filter(ExtractionFilterCriterion.isNotEmpty).length
      )
  ...
  hasFilterCriteria(sheetName?: string): boolean {
    ...
    return sheetCriteriaForm && sheetCriteriaForm.controls.map((c) => c.value).findIndex(ExtractionFilterCriterion.isNotEmpty) !== -1;
  }
