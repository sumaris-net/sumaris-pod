...
export class CatchBatchForm extends MeasurementValuesForm<Batch> implements OnInit {
  ...
  ngOnInit() {
    ...
    firstNotNilPromise(this.$pmfms).then((pmfms) => {
      ...
      this.$weightAndOtherPmfms.next(
        pmfms.filter((p) => (p.label && p.label.indexOf('_WEIGHT') > 0) || (p.label.indexOf('ON_DECK_') === -1 && p.label.indexOf('SORTING_') === -1))
      );
    ...
    this.registerSubscription(this._onValueChanged.subscribe((_) => (this.data.label = this._acquisitionLevel)));
  }
}
