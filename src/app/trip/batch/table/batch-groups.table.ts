...
import { TaxonGroupRef } from '@app/referential/services/model/taxon.model';
...
@Component({
  ...
  providers: [{ provide: ValidatorService, useExisting: BatchGroupValidatorService }],
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class BatchGroupsTable extends BatchesTable<BatchGroup> implements OnInit {
  ...
  constructor(injector: Injector, protected platform: PlatformService) {
    super(
      injector,
  ...
  async autoFillTable(opts?: { defaultTaxonGroups?: string[] }) {
    ...
    try {
      ...
      const rowsTaxonGroups = ((await this.dataSource.getRows()) || [])
        .map((r) => r.currentData)
  ...
  protected mapPmfms(pmfms: DenormalizedPmfmStrategy[]): DenormalizedPmfmStrategy[] {
    ...
    const fakePmfms = (this.dynamicColumns || []).map((col) =>
  ...
  protected computeDynamicColumns(qvPmfm: IPmfm): ColumnDefinition[] {
    ...
    if (isNil(qvPmfm)) {
      ...
      if (!this.weightMethodForm && this.weightPmfmsByMethod[MethodIds.ESTIMATED_BY_OBSERVER]) {
        ...
        this.registerSubscription(this.weightMethodForm.valueChanges.subscribe((_) => this.markAsDirty()));
      }
  ...
  protected getDisplayColumns(): string[] {
    ...
    const dynamicColumnKeys = (this.dynamicColumns || [])
      .map((c) => ({
        key: c.key,
        rankOrder: c.rankOrder + ((inverseOrder && ((c.key.endsWith('_WEIGHT') && 1) || (c.key.endsWith('_INDIVIDUAL_COUNT') && -1))) || 0),
      }))
  ...
  async onSubBatchesClick(event: UIEvent, row: TableElement<BatchGroup>, opts?: { showParent?: boolean; emitLoaded?: boolean }) {
  ...
  async openSubBatchesModal(
    ...
    const onNewParentClick = showParentGroup
      ? async () => {
    ...
    const availableParents = (showParentGroup ? this.dataSource.connect(null) : defer(() => this.dataSource.getRows())).pipe(
  ...
  async openSelectColumnsModal(event?: UIEvent) {
    ...
    let columns = (userColumns || []).concat(hiddenColumns).map((name) => {
      const label = name === 'individualCount' ? 'TRIP.BATCH.TABLE.INDIVIDUAL_COUNT' : name === 'weight' ? 'TRIP.BATCH.TABLE.WEIGHT' : '';
      return {
        name,
        label,
  ...
  async loadAvailableTaxonGroups(opts?: { defaultTaxonGroups?: string[] }): Promise<TaxonGroupRef[]> {
    ...
    const taxonGroups = ((await this.programRefService.loadTaxonGroups(this.programLabel)) || [])
      ...
      .filter(
        (taxonGroup) =>
          !defaultTaxonGroups || (taxonGroup.label && defaultTaxonGroups.findIndex((label) => taxonGroup.label.startsWith(label)) !== -1)
      )
