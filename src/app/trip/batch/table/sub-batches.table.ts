...
export class SubBatchesTable extends AppMeasurementsTable<SubBatch, SubBatchFilter> implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    if (this.inlineEdition) {
      // can be override bu subclasses

      // Create listener on column 'DISCARD_OR_LANDING' value changes
      this.registerSubscription(
        this.registerCellValueChanges('discard', 'measurementValues.' + PmfmIds.DISCARD_OR_LANDING.toString()).subscribe((value) => {
      ...
      this.registerSubscription(
        this.registerCellValueChanges('parentGroup', 'parentGroup').subscribe((parentGroup) => {
  ...
  protected mapPmfms(pmfms: IPmfm[]) {
    ...
    const parentTaxonGroupIds = (this._availableParents || []).map((parent) => parent.taxonGroup && parent.taxonGroup.id).filter(isNotNil);
    if (isNotEmptyArray(parentTaxonGroupIds)) {
      pmfms = pmfms.map((pmfm) => {
        if (
          ...
          pmfm.taxonGroupIds.some((id) => parentTaxonGroupIds.includes(id))
        ) {
