...
const sortByDateTimeFn = (n1: VesselPosition, n2: VesselPosition) =>
  n1.dateTime.isSame(n2.dateTime) ? 0 : n1.dateTime.isAfter(n2.dateTime) ? 1 : -1;
...
export class Operation extends DataEntity<Operation, number, OperationAsObjectOptions, OperationFromObjectOptions> {
  ...
  asObject(opts?: OperationAsObjectOptions): any {
    ...
    target.positions = [target.startPosition, target.endPosition].filter((p) => p && p.dateTime).map((p) => p && p.asObject(opts)) || undefined;
  ...
  fromObject(source: any, opts?: OperationFromObjectOptions) {
    ...
    if (
      this.endPosition &&
      this.endPosition.dateTime &&
      this.startPosition &&
      this.endPosition.dateTime.isSameOrBefore(this.startPosition.dateTime)
    ) {
...
export class OperationGroup extends DataEntity<OperationGroup> implements IWithProductsEntity<OperationGroup>, IWithPacketsEntity<OperationGroup> {
  ...
  asObject(opts?: DataEntityAsObjectOptions & { batchAsTree?: boolean }): any {
    ...
    // Physical gear
    target.physicalGear =
      this.physicalGear && this.physicalGear.asObject({ ...opts, ...NOT_MINIFY_OPTIONS /*Avoid minify, to keep gear for operations tables cache*/ });
...
export class VesselPosition extends DataEntity<VesselPosition> {
  ...
  equals(other: VesselPosition): boolean {
    return (
      super.equals(other) ||
      (this.dateTime === other.dateTime && ((!this.operationId && !other.operationId) || this.operationId === other.operationId))
    );
  }
}
