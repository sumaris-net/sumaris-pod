...
const sortByDateOrIdFn = (n1: Landing, n2: Landing) =>
  n1.dateTime.isSame(n2.dateTime) ? (n1.id === n2.id ? 0 : n1.id > n2.id ? 1 : -1) : n1.dateTime.isAfter(n2.dateTime) ? 1 : -1;
  ...
  implements IEntitiesService<Landing, LandingFilter, LandingServiceWatchOptions>, IEntityService<Landing>
{
  ...
  watchAll(
    ...
    const offline =
      ...
      (dataFilter &&
        ((dataFilter.synchronizationStatus && dataFilter.synchronizationStatus !== 'SYNC') ||
          dataFilter.observedLocationId < 0 ||
          dataFilter.tripId < 0)) ||
      false;
    if (offline) {
    ...
    const query = fullLoad ? LandingQueries.loadAllFullWithTotal : withTotal ? LandingQueries.loadAllWithTotal : LandingQueries.loadAll;
  ...
  async saveAll(entities: Landing[], opts?: LandingSaveOptions): Promise<Landing[]> {
    ...
    const localEntities = entities.filter(
      (entity) => entity && (entity.id < 0 || (entity.synchronizationStatus && entity.synchronizationStatus !== 'SYNC'))
    );
  ...
  watchAllLocally(
    ...
    return this.entities.watchAll<Landing>(entityName, variables, { fullLoad: opts && opts.fullLoad }).pipe(
      map(({ data, total }) => {
        ...
        // Compute rankOrder, by tripId or observedLocationId
        if (!opts || opts.computeRankOrder !== false) {
          this.computeRankOrderAndSort(entities, offset, total, sortBy !== 'id' ? sortBy : 'rankOrder', sortDirection, dataFilter);
        }
