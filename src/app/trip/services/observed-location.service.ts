...
const ObservedLocationMutations = {
  ...
  terminate: gql`
    mutation TerminateObservedLocation($data: ObservedLocationVOInput!) {
  ...
  validate: gql`
    mutation ValidateObservedLocation($data: ObservedLocationVOInput!) {
  ...
  unvalidate: gql`
    mutation UnvalidateObservedLocation($data: ObservedLocationVOInput!) {
    ...
    IDataSynchroService<ObservedLocation, number, ObservedLocationLoadOptions>
{
  ...
  watchAll(
    ...
    // Load offline
    const offlineData =
      this.network.offline || (dataFilter && dataFilter.synchronizationStatus && dataFilter.synchronizationStatus !== 'SYNC') || false;
    if (offlineData) {
    ...
    try {
      ...
      if (id < 0) {
        ...
        if (opts && opts.withLanding) {
          data.landings = await this.entities.loadAll<Landing>(
            Landing.TYPENAME,
            {
  ...
  async deleteAllLocally(
    ...
    await chainPromises(
      localEntities.map((entity) => async () => {
        // Load children
        const res = await this.landingService.loadAllByObservedLocation(
  ...
  async synchronize(entity: ObservedLocation, opts?: ObservedLocationSaveOptions): Promise<ObservedLocation> {
    ...
    // Fill landings
    const res = await this.landingService.loadAllByObservedLocation({ observedLocationId: localId }, { fullLoad: true, rankOrderOnPeriod: false });
  ...
  protected getImportJobs(opts: { maxProgression: undefined }): Observable<number>[] {
    ...
    if (landingFilter) {
      return [
        ...
        JobUtils.defer(
          (p, o) =>
            this.landingService.executeImport(p, {
              ...o,
