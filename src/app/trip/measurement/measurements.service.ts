...
@Directive()
export class MeasurementsDataService<T extends IEntityWithMeasurement<T>, F> implements IEntitiesService<T, F>, OnDestroy {
  ...
  async saveAll(data: T[], options?: any): Promise<T[]> {
    ...
    const dataToSaved = data.map((json) => {
      ...
      entity.measurementValues = Object.assign(
        ...
        MeasurementValuesUtils.normalizeValuesToModel(json.measurementValues, pmfms)
      );
      return entity;
    });
  ...
  private async applyPmfms(pmfms: IPmfm[] | Observable<IPmfm[]>) {
    ...
    if (pmfms instanceof Array && pmfms !== this.$pmfms.getValue()) {
      ...
      if (this._debug)
        console.debug(
          `[meas-service] Pmfms loaded for {program: '${this.programLabel}', acquisitionLevel: '${this._acquisitionLevel}', strategyLabel: '${this._strategyLabel}'}`,
          pmfms
        );
