...
export class ObservedLocationsPage extends AppRootTable<ObservedLocation, ObservedLocationFilter> implements OnInit {
  ...
  constructor(
    ...
    super(
      ...
      modalCtrl,
      settings,
      RESERVED_START_COLUMNS.concat(['quality', 'program', 'location', 'startDateTime', 'observers', 'comments']).concat(RESERVED_END_COLUMNS),
      dataService,
  ...
  async deleteSelection(event: UIEvent): Promise<number> {
    ...
    if (isNotEmptyArray(observations)) {
      ...
      if (samplesCount > 0) {
        const messageKey =
          observations.length === 1 ? 'OBSERVED_LOCATION.CONFIRM.OBSERVATION_HAS_SAMPLE' : 'OBSERVED_LOCATION.CONFIRM.OBSERVATIONS_HAS_SAMPLE';
