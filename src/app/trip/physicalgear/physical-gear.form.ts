...
export class PhysicalGearForm extends MeasurementValuesForm<PhysicalGear> implements OnInit {
  ...
  ngOnInit() {
    ...
    this.form
      .get('gear')
      .valueChanges.pipe(filter((value) => ReferentialUtils.isNotEmpty(value) && !this.loading))
      .subscribe((value) => {
  ...
  protected async safeSetValue(
    data: PhysicalGear,
    opts?: { emitEvent?: boolean; onlySelf?: boolean; normalizeEntityToForm?: boolean }
  ): Promise<void> {
    if (data && ReferentialUtils.isNotEmpty(data.gear)) {
      this.gearId = data.gear.id;
    }
