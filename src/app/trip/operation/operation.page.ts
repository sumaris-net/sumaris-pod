...
export class OperationPage extends AppEntityEditor<Operation, OperationService> implements OnInit, AfterViewInit, OnDestroy {
    ...
    this.registerSubscription(
      this.samplesTable.dataSource.datasourceSubject
        ...
        .subscribe((samples) => {
          ...
          const availableParents = (samples || []).filter((s) => isNotNil(s.measurementValues[PmfmIds.TAG_ID.toString()]));
          ...
          if (isNotNil(isSamplingControl)) {
            this.registerSubscription(
              isSamplingControl.valueChanges
                .pipe(debounceTime(400), startWith<any, any>(isSamplingControl.value), filter(isNotNil), distinctUntilChanged())
  ...
  protected async computeTitle(
    ...
    if (this.platform.mobile) {
      return (titlePrefix +
        (await this.translate
          .get('TRIP.OPERATION.EDIT.TITLE_NO_RANK', {
  ...
  async onLastOperationClick(event: UIEvent, id: number): Promise<any> {
    ...
    const savePromise: Promise<boolean> =
      this.isOnFieldMode && this.dirty && this.valid
  ...
  setValue(data: Operation) {
    ...
    // Get all samples
    const samples = ((data && data.samples) || []).reduce(
      (res, sample) => (!sample.children ? res.concat(sample) : res.concat(sample).concat(sample.children)),
      []
    );
    ...
    this.individualMonitoringTable.availableParents = this.samplesTable.value.filter(
      (s) => s.measurementValues && isNotNil(s.measurementValues[PmfmIds.TAG_ID])
    );
    this.individualMonitoringTable.value = samples.filter(
      (s) => s.label && s.label.startsWith(this.individualMonitoringTable.acquisitionLevel + '#')
    );
  ...
  isCurrentData(other: IEntity<any>): boolean {
    return (this.isNewData && isNil(other.id)) || (this.data && this.data.id === other.id);
  }
  ...
  protected computeUsageMode(operation: Operation): UsageMode {
    return this.settings.isUsageMode('FIELD') &&
      (isNil(this.trip) || (isNotNil(this.trip.departureDateTime) && this.trip.departureDateTime.diff(moment(), 'day') < 15))
      ? 'FIELD'
      : 'DESK';
  }
  ...
  protected async getValue(): Promise<Operation> {
    ...
    if (this.showSampleTables) {
      ...
      const subSamples = (this.individualMonitoringTable.value || []).concat(this.individualReleaseTable.value || []);
      data.samples = (this.samplesTable.value || []).map((sample) => {
        // Add children
  ...
  protected canUserWrite(data: Operation): boolean {
    return !!data && this.trip && isNil(this.trip.validationDate) && this.tripService.canUserWrite(this.trip);
  }
  ...
  protected async setDefaultTaxonGroups(enable: boolean) {
    ...
    if (qvMeasurement && ReferentialUtils.isNotEmpty(qvMeasurement.qualitativeValue)) {
      ...
      const tripPmfms = await this.programRefService.loadProgramPmfms(this.$programLabel.getValue(), {
